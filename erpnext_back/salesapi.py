...
@frappe.whitelist()
def makeTemplate(mso_id,shipping_date,delivery_date,pallet_weight,usd,template_name):
...
def makeso1(poname,ps):
	...
	check=frappe.db.sql("""SELECT `name` FROM `tabSales Order` WHERE master_so_id=%s """,poname)
	if not check:
