...
@frappe.whitelist()
def get_sales_order(city_name=None, delivery_date=None, limit=50, status=None, vehicle=None, driver=None, sort='so.customer' ):
        ...
        if city_name:
                query += """ AND so.territory LIKE  %(city_name)s  """
        ...
        query += """ order by %s ASC LIMIT %s  """ % (cstr(sort),cint(limit))

        data = frappe.db.sql(query, { "city_name": city_name, "delivery_date": delivery_date,  "limit": limit, "status": status, "vehicle": vehicle, "driver": driver, "sort": sort }, as_dict=1)
...
def get_sales_territory():
	list = []
	data=frappe.db.sql("""select city from `tabAddress` group by city""")
	for acc in data:
		list.append(acc[0])
...
@frappe.whitelist()		
def get_sales_territoryo():
	list = []
	data=frappe.db.sql("""select name from `tabTerritory`""")
	for acc in data:
		data1=frappe.db.sql("""select name from `tabTerritory` where parent_territory=%s""",acc[0])
		if not data1:
			list.append(acc[0])
...
def get_context(context):
	context["google_maps_api_key"] = (frappe.db.get_single_value("Google Settings", "api_key")
