	...
	if has_batch_no:
		...
		if set_batch==False:
			...
			if float(total)<float(qty):
				...
				frappe.throw(_("Row #{0}: Insufficient Batch Qty To Fullfill Order Quantity. Batch's Qty is {1} and Order Qty is {2}.").format(itemdoc.idx,total,qty))
...
def itemAddUpdate(name,batch_no,qty=None,flag=False):
	if flag==True:
		quntity=0
		item_doc=frappe.get_doc("Sales Invoice Item",name)
		if qty==None:
...
@frappe.whitelist()
def validateSalesInvoice(self,method):
	if self.update_stock:
		for row in self.items:
			batches = get_batches(row.item_code, row.warehouse, row.qty)
			total=0
			sitem = frappe.get_doc("Item", row.item_code)
			
			if sitem.has_batch_no:
				for batch in batches:
					total=cint(total)+cint(batch.qty)

				if cint(total)<cint(row.qty):
					#frappe.throw("Insufficient All Batch Qty To Fullfill Order Quantity")
					frappe.throw(_("Row #{0}: Insufficient Batch Qty To Fullfill Order Quantity. Batch's Qty is {1} and Order Qty is {2}.").format(row.idx,total,row.qty))
		#frappe.msgprint("Validation Called")


@frappe.whitelist()
def test_fun(self,method):
