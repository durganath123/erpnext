...
def getData(pricelist_obj,filters):
	query="select name from `tabItem` where disabled=0 and is_sales_item=1"
	fil=json.loads(filters)
		
	if not fil['item_code']==None and not fil['item_group']==None:
		item_list=frappe.db.sql("""select tabItem.name,ig.group_color from `tabItem` LEFT JOIN `tabItem Group` as ig ON ig.name=tabItem.item_group where disabled=0 and item_code=%s and item_group=%s and is_sales_item=1""",(fil['item_code'],fil['item_group']))
	elif not fil['item_group']==None:
		item_list=frappe.db.sql("""select tabItem.name,ig.group_color from `tabItem` LEFT JOIN `tabItem Group` as ig ON ig.name=tabItem.item_group where disabled=0 and item_group=%s and is_sales_item=1""",fil['item_group'])
	elif not fil['item_code']==None:
		item_list=frappe.db.sql("""select tabItem.name,ig.group_color from `tabItem` LEFT JOIN `tabItem Group` as ig ON ig.name=tabItem.item_group where disabled=0 and item_code=%s and is_sales_item=1""",fil['item_code'])
	else:
		item_list=frappe.db.sql("""select tabItem.name,ig.group_color from `tabItem` LEFT JOIN `tabItem Group` as ig ON ig.name=tabItem.item_group where disabled=0 and is_sales_item=1""")
