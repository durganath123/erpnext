...
@frappe.whitelist(allow_guest=True)
def itemList():
	item_list=frappe.get_all("Item",filters={'has_variants':0},fields=["item_name", "item_code", "standard_rate", "weight_per_unit", "item_group","description","website_image","show_in_website"])
	if item_list:
		return item_list
	else:	
		return _(False)
