    ...
    for (addCol = 0; addCol < colCount; addCol++) {
        if (addCol == 0) {
            ...
            if (addCol == (colCount - 1)) {
                newRow += '<td data-row="' + row + '" data-col="' + addCol + '" class="hidecol"><span class="wgtitotal"></span><span class="gwgtitotal"></span></td>';
            } else if (addCol == (colCount - 2)) {
                newRow += '<td data-row="' + row + '" data-col="' + addCol + '"><span class="pltitotal"></span></td>';
            } else if (addCol == (colCount - 3)) {
                newRow += '<td data-row="' + row + '" data-col="' + addCol + '"><span class="qtytotal"></span></td>';
            } else if (addCol == (colCount - 4)) {
                newRow += '<td data-row="' + row + '" data-col="' + addCol + '"><span class="avgitotal"></span></td>';
            } else if (addCol == (colCount - 5)) {
                newRow += '<td data-row="' + row + '" data-col="' + addCol + '" class="hidecol"><span class="itotal"></span></td>';
            } else {
                newRow += '<td data-row="' + row + '" data-col="' + addCol + '"><input type="hidden" class="iname' + addCol + '" id="iname" name="iname' + addCol + '" value=""><input type="hidden"   id="rqty" name="rqty' + addCol + '" class="rqty' + addCol + '" value="0"><input type="text"   id="iqty" name="iqty' + addCol + '" class="iqty' + addCol + '" value=""><input  id="iprice"  type="text" class="iprice' + addCol + '" name="iprice' + addCol + '" value=""></td>';
            }
        }
    }
    //close the row
...
jQuery.fn.updatedata = function() {
    ...
    for (var i = 5; i < (colCount - 5); i++) {
		...
		{
			iprice = parseFloat(row.find('.iprice' + i).val());
	...
	gwgtitotal_total = gwgtitotal_total + (pltitotal_total.toFixed(2) * pallet_weight);
        ...
        $.ajax({
            ...
            success: function(r) {
				...
				$("#myTable").find(".sitems").each( function()
					{
						var rowf = $(this).parents('.item-row');
						if(rowf.find('.item_code').val() != '') 
						{
							items_list.push(rowf.find('.item_code').val());
						}
					});
				...
				if(jQuery.inArray(r.data[i]['item_code'], items_list) < 0)
				{
                    ...
                    if (r.data[i]['item_code'].toUpperCase().includes(val.toUpperCase())) {
                        ...
                        b.addEventListener("click", function(e) {
							...
							for (var i = 5; i < (colCount - 5); i++) 
							{
								...
								var suppliername = $("#isupplier"+i).val();
									...
									frappe.call({
										...
										callback: function(r) {
											...
											var prc1 = parseFloat(r.message[0].price).toFixed(2);
											var row1 = $(r.message[0].row).parents('.item-row');
											row1.find(r.message[0].column).val(prc1);
                                ...
                                $('.stotal' + i).html(parseFloat(stotal).toFixed(2));
                                $('.qtotal' + i).html(parseFloat(qtotal).toFixed(2));
                                stotal = 0;
                                qtotal = 0;
                            }
                            avgitotal = avgitotal + parseFloat(itotal / qtytotal);
            ...
            success: function(r) {
                for (i = 0; i < r.data.length; i++) {
                    ...
                    if (r.data[i]['supplier_name'].toUpperCase().includes(val.toUpperCase())) {
						...
						b.innerHTML += '<input type="hidden" class="citmname" value="' + r.data[i]['name'] + '"><input type="hidden" class="citmid" value="' + r.data[i]['name'] + '"><input type="hidden" class="citmpl" value="' + dpl + '"><input type="hidden" class="citmcurrency" value="' + r.data[i]['default_currency'] + '"><span style="display:none"  class="cidesc">' + r.data[i]['supplier_name'] + '</span>';
						
                       // b.innerHTML += "<input type='hidden' class='citmname' value='" + r.data[i]['name'] + "'><input type='hidden' class='citmid' value='" + r.data[i]['name'] + "'><input type='hidden' class='citmpl' value='" + r.data[i]['default_price_list'] + "'><input type='hidden' class='citmcurrency' value='" + r.data[i]['default_currency'] + "'><span style='display:none'   class='cidesc'>" + r.data[i]['supplier_name'] + "</span>";
                        /*execute a function when someone clicks on the item value (DIV element):*/
                        b.addEventListener("click", function(e) {
                            /*insert the value for the autocomplete text field:*/
                            sinp.value = this.getElementsByClassName("citmname")[0].value; //alert(sinp.value );

                            var sidcell = $("#" + idd).parents('td');
							if(!this.getElementsByClassName("citmpl")[0].value || this.getElementsByClassName("citmpl")[0].value == null) { var df_pl = ""; } else {  var df_pl = this.getElementsByClassName("citmpl")[0].value;  }
                            sidcell.find('.isupplierid').val(this.getElementsByClassName("citmid")[0].value);
                            sidcell.find('.default_price_list').val(df_pl);
                            sidcell.find('.default_currency').val(this.getElementsByClassName("citmcurrency")[0].value);
							...
							$('.sitems').each(function() {
								...
								if(row.find('.item_code').val() != '')
								{
									frappe.call({
										...
										callback: function(r) {
											console.log(r.message);
											var prc = parseFloat(r.message[0].price).toFixed(2);
											var row1 = $(r.message[0].row).parents('.item-row');
											row1.find(r.message[0].column).val(prc);
