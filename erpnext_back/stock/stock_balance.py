...
def repost_all_stock_vouchers(from_date=None, repost_gle=False):
	import datetime
	import os
	import json
	...
	for voucher_type, voucher_no in vouchers:
		try:
			...
			if voucher_type == "Stock Entry" and doc.purpose in ["Manufacture", "Repack", "Material Transfer"]:
...
def repost_stock_mismatch_gle(from_date=None, to_date=None):
	from erpnext.stock.report.stock_and_account_value_comparison.stock_and_account_value_comparison import get_data
	mismatch_data = get_data({"from_date": from_date, "to_date": to_date})
