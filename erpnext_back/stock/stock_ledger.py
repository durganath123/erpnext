...
def make_sl_entries(sl_entries, is_amended=None, allow_negative_stock=False, via_landed_cost_voucher=False):
	if sl_entries:
		...
		for sle in sl_entries:
			...
			if sle.get("actual_qty") or (sle.voucher_type == "Stock Reconciliation" and cint(sle.get('reset_rate'))):
...
class update_entries_after(object):
	...
	def process_sle(self, sle):
		...
		if serial_nos:
			...
			if sle.voucher_type=="Stock Reconciliation" and sle.reset_rate:
				stock_value_difference = self.get_stock_reconciliation_reset_values(sle)
			else:
		...
		if stock_value_difference is None:
	...
	def get_stock_reconciliation_reset_values(self, sle):
		...
		stock_value_difference = self.get_stock_reconciliation_stock_value_difference(sle)
		new_stock_value = self.get_stock_value_from_stock_value_difference(sle, batch_sle=True, batch_exclude=True)
		new_stock_value += self.batch_data.batch_stock_value
		...
		return stock_value_difference

	def get_stock_reconciliation_stock_value_difference(self, sle):
		previous_stock_value = self.get_stock_value_from_stock_value_difference(sle, batch_sle=self.batch_wise_valuation)
		new_stock_value = self.batch_data.batch_stock_value if self.batch_wise_valuation else self.stock_value

		stock_value_difference = new_stock_value - previous_stock_value
		return stock_value_difference

	def get_stock_value_from_stock_value_difference(self, sle, batch_sle, batch_exclude=False):
		batch_condition = ""
		if batch_sle:
			if batch_exclude:
				batch_condition = "and batch_no != %(batch_no)s"
			else:
				batch_condition = "and batch_no = %(batch_no)s"

		previous_stock_value = frappe.db.sql("""
			select sum(stock_value_difference)
			from `tabStock Ledger Entry`
			where item_code = %(item_code)s and warehouse = %(warehouse)s
				and (posting_date, posting_time, creation) <= (%(posting_date)s, %(posting_time)s, %(creation)s)
				and name != %(name)s and ifnull(is_cancelled, 'No') = 'No'
				{0}
		""".format(batch_condition), sle)

		previous_stock_value = flt(previous_stock_value[0][0]) if previous_stock_value else 0
