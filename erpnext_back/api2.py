...
@frappe.whitelist()
def supplierPriceList(row,column,supplier,item,price_list):
	result_list2=[]
	d={}
	d["row"]=row
	d["column"]=column
	if not supplier or not price_list:
		d["price"]=0
	else:
		data=frappe.db.sql("""select price_list_rate from `tabItem Price` where price_list=%s and buying=1 and item_code=%s """,(price_list,item))
...
@frappe.whitelist()
def customerPriceList(row,column,supplier,item,price_list):
	result_list2=[]
	d={}
	d["row"]=row
	d["column"]=column
	if not supplier or not price_list:
		...
		data=frappe.db.sql("""select price_list_rate from `tabItem Price` where price_list=%s and selling=1 and item_code=%s """,(price_list,item))
...
@frappe.whitelist()
def customerAddress(column,supplier):
	result_list2=[]
	d={}
	d["column"]=column
	
	data1=frappe.db.sql("""select ad.name,ad.address_title from `tabAddress` as ad inner join `tabDynamic Link` as dl on ad.name=dl.parent where dl.link_doctype='Customer' and ad.is_shipping_address=1 and dl.link_name=%s """,supplier)
	
	if data1:
		d["address"]=data1[0][0]
		d["address_title"]=data1[0][1]
	else:
		d["address"]=""
		d["address_title"]=""
...
@frappe.whitelist()
def customerPricelist(column,supplier):
	result_list2=[]
	d={}
	d["column"]=column
	
	data1=frappe.db.sql("""select cu.default_price_list from `tabCustomer` as cu where cu.name=%s """,supplier)
	
	if data1:
		d["default_price_list"]=data1[0][0]
	else:
		d["default_price_list"]="STD"
	...
	if d:
		for row in data["master_purchase_order_item"]:
			...
			data3=frappe.get_doc({
								...
								"default_currency":  getCurrency(row["supplier"]),
...
def makepo1(poname,ps):
	...
	count=0
	i=0
	check=frappe.db.sql("""SELECT `name` FROM `tabPurchase Order` WHERE master_po_id=%s """,poname)
	if not check:
		for row in data["details"]:
			...
			data3=frappe.get_doc({
								...
								"shipping_date":row[0]["shipping_date"],
								"supplier":row[0]["supplier"],
								"buying_price_list":row[0]["default_price_list"],
								...
								"base_grand_total": row[0]["base_grand_total"]
							})
